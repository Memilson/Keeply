...
class FsController {
  ...
  void handleFsList(WsCommand cmd) {
    ...
    try {
      ...
      _ws.sendJson({'type': 'fs.list', 'requestId': cmd.requestId, 'path': targetPath, 'parentPath': dir.parent.path, 'truncated': count >= 120, 'items': entries.map((e) => e.toJson()).toList()});
    } catch (e) {
  ...
  void handleFsDisks(WsCommand cmd) {
    ...
    _ws.sendJson({'type': 'fs.disks', 'requestId': cmd.requestId, 'items': entries.map((e) => e.toJson()).toList()});
  }
}
