...
class WsController extends ChangeNotifier {
  ...
  /// Inicializa em modo offline — configura estado local sem WebSocket.
  void initOffline(WsConfig config, AgentIdentity identity) {
    _config = config;
    _identity = identity;
    _offlineMode = true;
    _connected = false;
    ...
    addLog('Device ID: ${identity.deviceId}');
    addLog('Scan source: ${_appState.source}');
    notifyListeners();
  }

  void initLocalState(WsConfig config, AgentIdentity identity) {
    _config = config;
    _identity = identity;
    _offlineMode = false;
    _connected = false;
    ...
    _appState.archive = '${config.identityDir}/keeply.kipy';
    _appState.restoreRoot = '${config.identityDir}/restore';
    addLog('Agente inicializado localmente (online-first).');
    addLog('Device ID: ${identity.deviceId}');
    notifyListeners();
  }
  ...
  Future<void> _doConnect() async {
    try {
      _channel?.sink.close();
      final uri = Uri.parse(
        '${_config.url}?deviceId=${Uri.encodeComponent(_config.agentId)}',
      );
  ...
  void _onDisconnect() {
    ...
    _channel = null;
    notifyListeners();
    if (_shouldReconnect && !_offlineMode) _scheduleReconnect();
  }
  ...
  void _startKeepAlive() {
    ...
    _keepAliveTimer = Timer.periodic(
      const Duration(milliseconds: _kKeepAliveMs),
      (_) {
  ...
  void _executeCommand(WsCommand cmd) {
    switch (cmd.type) {
      ...
      default:
        sendJson({
          'type': 'error',
          'message': 'Comando nao suportado: ${cmd.type}',
        });
    }
  }
  ...
  /// Método público para sub-controllers dispararem notificação de UI.
  void refresh() => notifyListeners();

  void sendJson(Map<String, dynamic> payload) {
    if (_offlineMode) return; // Não envia nada em modo offline
  ...
  /// Ativa modo offline desconectando do backend.
  void goOffline() {
    disconnect();
    _offlineMode = true;
    addLog('Modo offline ativado');
