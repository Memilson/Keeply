...
class ConfigController {
  ...
  void handleConfigSource(WsCommand cmd) {
    ...
    _ws.appState.scanScope = ScanScope(
        id: 'custom',
        label: 'Custom',
        requestedPath: cmd.path,
  ...
  void handleConfigArchive(WsCommand cmd) {
    if (cmd.path.isEmpty) {
      _ws.sendJson(
          {'type': 'error', 'message': 'archive nao pode ser vazio.'});
      return;
    }
  ...
  void handleConfigRestoreRoot(WsCommand cmd) {
    if (cmd.path.isEmpty) {
      _ws.sendJson(
          {'type': 'error', 'message': 'restoreRoot nao pode ser vazio.'});
      return;
    }
  ...
  ScanScope _resolveScanScope(String scopeId) {
    ...
    if (entry == null) {
      return ScanScope(
          id: 'home', label: 'Home', resolvedPath: '/storage/emulated/0');
    }
    return ScanScope(
        id: id, label: entry['label']!, resolvedPath: entry['path']!);
  }
}
